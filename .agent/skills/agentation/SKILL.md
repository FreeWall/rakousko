...
6. **Recommend MCP server setup**
   ...
   - Recommend one of the following approaches:
     - **Universal (supports 9+ agents including Claude Code, Cursor, Codex, Windsurf, etc.):**
       See [add-mcp](https://github.com/neondatabase/add-mcp) — run `npx add-mcp "npx -y agentation-mcp server"` and follow the prompts to add `agentation-mcp` as an MCP server
