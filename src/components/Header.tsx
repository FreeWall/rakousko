    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 mx-auto flex max-w-4xl flex-col gap-6">
          ...
          {isWeatherExpanded && (
            <div className="w-full space-y-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
              {/* Weekly section */}
              <div>
                ...
                <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-7">
                  {displayWeather.map((w, idx) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`text-[10px] font-bold ${isCurrent ? 'text-emerald-400' : 'text-slate-400'}`}
                        >
                          {w.day}
                        </span>
                        {w.iconUrl ? (
                        ...
                        <div
                          className="mt-1 max-w-full truncate text-[8px] text-slate-500"
                          title={w.desc}
                        >
                          {w.desc}
                        </div>
                ...
                {isHourlyLoading ? (
                  ...
                  <div className="flex scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-slate-950/10 gap-2.5 overflow-x-auto pt-0.5 pb-2">
                    {hourlyForSelectedDay.map((h, idx) => (
                      <div
                        key={idx}
                        className="flex min-w-[64px] flex-col items-center justify-center rounded-xl border border-slate-800/50 bg-slate-900/30 p-2 text-center transition-all hover:bg-slate-900/60"
                      >
                        ...
                        {h.rainProb ? (
                          <span className="mt-0.5 flex items-center gap-0.5 text-[8px] font-semibold text-sky-400">
                            <Droplets className="h-2 w-2" /> {h.rainProb}
                          </span>
                        ) : (
                          <span className="mt-0.5 text-[8px] text-transparent select-none">-</span>
                        )}
                      </div>
                    ))}
                  </div>
